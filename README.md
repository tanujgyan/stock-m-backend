...
* **Stock Data:** Retrieve real-time and
  historical stock information from an external
  API (Polygon.io).
...
* Python 3.8+
* PostgreSQL database
* Polygon.io API key (or another financial data
  API)
...
4. **Set up Environment Variables:**
   ...
   FINANCIAL_API_KEY=your_polygon_api_key
...
* `GET /stocks/{symbol}`: Retrieve data for a
  specific stock.
*
`GET /stocks/{symbol}/historical/{date_from}/{date_to}`:
